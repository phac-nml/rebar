...
pub fn linelist(
    ...
    // strain
    let strain = best_match.sequence_id.to_string();
    ...
    // genome_length
    let genome_length = dataset.reference.genome_length.to_string();
    ...
    // cli version
    row[table.header_position("cli_version")?] = env!("CARGO_PKG_VERSION").to_string();
